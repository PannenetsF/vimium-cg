How Vimium CG Respects Your Privacy
===================================

Vimium CG supports both export and cloud-based synchronization of your settings (including key mappings and search
engines). None of your browsing behavior is included.

By default, Vimium CG only downloads synced settings during the first installation, and if no synced settings are found,
...
All information handled by Vimium CG, including history, bookmarks, page titles and URLs, will only be used locally.
...
Vimium CG requires these permissions:
...
* **`webNavigation`**: to match an incoming URL, confirming whether Vimium CG should be enabled or disabled on the page.
...
* **`notifications`**: to show a notification whenever Vimium CG is upgraded.
...
* **`storage`**: its syncing functionality is **not in use** by default. However, if you enable "`Sync settings with
  your current account for this browser`", Vimium CG will require **`storage.sync`**, to sync your settings items with
...
* The "**browsing activity**" listed above actually includes a number of separate permissions: `tabs`, `<all_urls>` and
  `webNavigation`.
* Vimium CG never deletes any browser history item, except on explicit user requests (such as <kbd>Shift+Delete</kbd>).


Other Information
===================

Vimium CG formerly overrode the "newtab" browser URL, to provide a better user experience on the `chrome://newtab` page.
That behavior was reversed in version 1.77.0, released in Sep 2019: Vimium CG no longer overrides this setting. If you'd
...
Vimium CG registers a search key (`v`) for browser's [omnibox](https://developer.chrome.com/extensions/omnibox) (the
address bar). This makes the box work just like Vomnibar in omni mode.

You may remove certain permissions without breaking Vimium CG (note that some commands might fail as a result):
...
* In versions previous to Chrome 37, no `sessions` functionality exists, so a few of Vimium CG's commands won't work.
